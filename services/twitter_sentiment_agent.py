# services/twitter_sentiment_agent.py - IMPROVED REAL TWEETS VERSION
from config import *
import requests
...
class TwitterSentimentAgent:
    ...
    def _search_tweets(self, query, count=15):
        """Fetch recent tweets using free public search (no API key)"""
        try:
            # Using a free public Twitter search endpoint (works reliably)
            ...
            for line in r.text.splitlines():
                if "<title>" in line and "</title>" in line:
                    ...
                    if len(title) > 10 and query.lower() in title.lower() or any(k in title.lower() for k in ["beat", "fda", "merger", "squeeze", "guidance"]):
                        titles.append(title)
            return titles[:count]
        except:
            return []
    ...
    def predict(self, symbols=None):
        ...
        for sym in symbols[:5]:
            tweets = self._search_tweets(sym, count=20)
            if not tweets:
            ...
            cat = sum(1 for t in tweets if any(k in t.lower() for k in ["earnings", "fda", "beat", "guidance", "squeeze", "merger"])) / len(tweets)
        ...
        entry = {
            ...
            "hot_stocks": {sym: round(stock_signals.get(sym, 0), 2) for sym in symbols[:5]}
        }
